...
public class APIVerticle extends AbstractVerticle {
    ...
    private void heroesConfig(RoutingContext routingContext) {
        JsonObject message = new JsonObject();
        eb.<JsonObject>send(EventTopic.GET_HEROES_DATA, message, reply -> {
            if (reply.succeeded()) {
                routingContext.response().setStatusMessage("OK").end(reply.result().body().encode());
            } else {
                routingContext.fail(reply.cause());
    ...
    private void buyHero(RoutingContext routingContext) {
        ...
        JsonObject message = new JsonObject()
                .put("playerId", body.getLong("player_id", null))
                .put("heroId", body.getLong("hero_id", -1L));

        eb.<JsonObject>send(EventTopic.BUY_STORE_HERO, message, reply -> {
            if (reply.succeeded()) {
                routingContext.response().setStatusMessage("OK").end(reply.result().body().encode());
            } else {
                routingContext.fail(reply.cause());
